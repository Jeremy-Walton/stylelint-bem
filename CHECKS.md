...
**Opt-in — not enabled by the recommended config.** Unlike the other four checks, this one governs how a file is structured rather than whether a class is valid BEM, so it must be turned on explicitly (see `README.md`).
